...
export default function SettingsPage() {
  ...
  const allServers = Array.isArray(allServersData)
    ? allServersData
  ...
  const handleConfirmBan = (reason?: string, durationHours?: number, durationMinutes?: number) => {
    if (memberToBan) {
      banUser.mutate(
        { serverId: currentServer.id, userId: memberToBan.id, reason, durationHours, durationMinutes },
        {
          onSuccess: () => {
      ...
      <div className="max-w-4xl mx-auto p-8 space-y-8">
        ...
        <Tabs defaultValue="servers">
          <TabsList className="bg-[#1E1F22] mb-6 w-full">
            <TabsTrigger value="servers" className="flex-1 px-2 sm:px-10">
              {t('servers')}
            </TabsTrigger>
            <TabsTrigger value="channels" className="flex-1 px-2 sm:px-10">
              {t('channels')}
            </TabsTrigger>
            <TabsTrigger value="members" className="flex-1 px-2 sm:px-10">
              {t('members')}
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="servers" className="space-y-4 outline-none">
            <ManagementCard
              ...
              onSave={(newName: string) =>
                updateServer.mutate({ id: currentServer.id, name: newName })
              }
