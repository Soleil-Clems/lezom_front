...
import { loginRequest, registerRequest, verifyOtpRequest, resendOtpRequest } from '@/requests/authRequest'
...
describe('authRequest', () => {
  ...
  it('loginRequest appelle POST auth/login', async () => {
    ...
    await loginRequest({ email: 'a@b.com', password: 'pass', token: '' })
    expect(customfetch.post).toHaveBeenCalledWith('auth/login', expect.objectContaining({ email: 'a@b.com' }))
  })

  it('registerRequest appelle POST users', async () => {
    vi.mocked(customfetch.post).mockResolvedValue({ id: 1 })
    await registerRequest({ email: 'a@b.com', password: 'pass', firstname: 'A', lastname: 'B', username: 'ab', birthdate: '2000-01-01', token: '' })
