...
describe('conversationRequest', () => {
  ...
  it('getAllConversationsRequest appelle GET conversations', async () => {
    ...
    expect(customfetch.get).toHaveBeenCalledWith('conversations?page=1&limit=20');
  });
  ...
  it('sendPrivateMessageRequest appelle POST conversations/:id/messages', async () => {
    ...
    expect(customfetch.post).toHaveBeenCalledWith('conversations/1/messages', {
      content: 'hi',
      type: 'text',
    });
  });
  ...
  it('updatePrivateMessageRequest appelle PATCH conversations/messages/:id', async () => {
    ...
    expect(customfetch.patch).toHaveBeenCalledWith('conversations/messages/5', {
      content: 'updated',
    });
  });
