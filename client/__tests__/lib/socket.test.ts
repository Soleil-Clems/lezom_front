...
describe('SocketManager', () => {
  ...
  it('connect retourne null si pas de token', () => {
    vi.mocked(useAuthStore.getState).mockReturnValue({ token: null, setToken: vi.fn(), logout: vi.fn() } as any)
    const result = socketManager.connect()
    expect(result).toBeUndefined()
  })

  it('connect crée un socket si token présent', () => {
    vi.mocked(useAuthStore.getState).mockReturnValue({ token: 'tok', setToken: vi.fn(), logout: vi.fn() } as any)
    const result = socketManager.connect()
