...
import { registerRequest, verifyOtpRequest, resendOtpRequest } from '@/requests/authRequest';
...
describe('useVerifyOtp callbacks', () => {
  ...
  it('onSuccess invalide authuser', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ email: 'a@b.com', otp: '123456' } as any);
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
  });
...
describe('useResendOtp callbacks', () => {
  ...
  it('onSuccess appelle toast.success', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ email: 'a@b.com' } as any);
    });
