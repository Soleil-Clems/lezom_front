...
describe('useUpdateServer callbacks', () => {
  ...
  it('onSuccess appelle toast.success', async () => {
    ...
    await act(async () => { await result.current.mutateAsync({ id: '1', name: 'New Name' }) })
    await waitFor(() => expect(vi.mocked(toast.success)).toHaveBeenCalledWith('Serveur mis à jour'))
  })

  it('onError appelle toast.error', async () => {
    vi.mocked(updateServerNameRequest).mockRejectedValue(new Error('Erreur'))
    const { result } = renderHookWithQuery(() => useUpdateServer())
    await act(async () => { try { await result.current.mutateAsync({ id: '1', name: '' }) } catch {} })
...
describe('useUpdateChannel callbacks', () => {
  ...
  it('onSuccess appelle toast.success', async () => {
    ...
    await act(async () => { await result.current.mutateAsync({ id: '1', name: 'general' }) })
