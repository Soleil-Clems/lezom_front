...
describe('middleware', () => {
  it('redirige vers /login si pas de token et route protégée', async () => {
    ...
    expect(NextResponse.redirect).toHaveBeenCalledWith(expect.objectContaining({ href: expect.stringContaining('/login') }))
  })

  it('redirige vers / si token présent et route publique', async () => {
    const req = makeRequest('/login', 'valid-token')
    await middleware(req as any)
    expect(NextResponse.redirect).toHaveBeenCalledWith(expect.objectContaining({ href: expect.stringContaining('/') }))
  })
