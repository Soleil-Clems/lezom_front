...
const ListContent = ({ memberships, online, offline, showClose = false, to, tc, ts }: any) => (
  <div className="flex flex-col h-full bg-[#2B2D31] text-zinc-300 w-full overflow-hidden border-l border-black/10">
    <div className="h-12 border-b border-black/20 flex items-center justify-between px-4 font-semibold text-sm shrink-0">
      <span className="opacity-70 text-[11px] uppercase tracking-widest font-bold">{tc("members", { count: memberships.length })}</span>
      {showClose && (
    ...
    <div className="flex-1 overflow-y-auto p-3 custom-scrollbar">
       {online.length > 0 && (
        <div className="mb-6">
          <p className="uppercase text-[11px] font-bold text-zinc-500 mb-2 px-2">{to("online")} — {online.length}</p>
      ...
      {offline.length > 0 && (
        <div>
          <p className="uppercase text-[11px] font-bold text-zinc-500 mb-2 px-2">{to("offline")} — {offline.length}</p>
    ...
    <>
      <Sidebar side="right" collapsible="none" className="hidden xl:flex w-60 border-l border-black/20 shrink-0">
        <SidebarContent className="bg-[#2B2D31]">
           <ListContent memberships={memberships} online={online} offline={offline} to={to} tc={tc} ts={ts} />
        </SidebarContent>
      </Sidebar>
      ...
      <div className="xl:hidden">
        <Sheet>
          ...
          <SheetContent side="right" className="p-0 w-[280px] bg-[#2B2D31] border-none [&>button]:hidden">
            <SheetHeader className="sr-only"><SheetTitle>{ts("membersList")}</SheetTitle></SheetHeader>
            <ListContent memberships={memberships} online={online} offline={offline} showClose={true} to={to} tc={tc} ts={ts} />
          </SheetContent>
        </Sheet>
      </div>
    </>
