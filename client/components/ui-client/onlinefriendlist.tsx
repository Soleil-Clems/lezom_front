...
const ListContent = ({ memberships, online, offline, showClose = false, to, tc, ts }: any) => (
  <div className="flex flex-col h-full bg-[#2B2D31] text-zinc-300 w-full overflow-hidden border-l border-black/10">
    <div className="h-12 border-b border-black/20 flex items-center justify-between px-4 font-semibold text-sm shrink-0">
      <span className="opacity-70 text-[11px] uppercase tracking-widest font-bold">
        {tc('members', { count: memberships.length })}
      </span>
      {showClose && (
    ...
    <div className="flex-1 overflow-y-auto p-3 custom-scrollbar">
      {online.length > 0 && (
        <div className="mb-6">
          <p className="uppercase text-[11px] font-bold text-zinc-500 mb-2 px-2">
            {to('online')} — {online.length}
          </p>
...
export function OnlineFriendsList({ serverId }: { serverId: string | number }) {
  ...
  const online = memberships.filter(
    (m: any) => m.members?.isActive === true && onlineUserIds.includes(m.members?.id),
  );

  const offline = memberships.filter(
    (m: any) => m.members?.isActive === false || !onlineUserIds.includes(m.members?.id),
  );
  ...
  return (
    <>
      <Sidebar
        side="right"
        collapsible="none"
        className="hidden xl:flex w-60 border-l border-black/20 shrink-0"
      >
        ...
        <Sheet>
          ...
          >
            <SheetHeader className="sr-only">
              <SheetTitle>{ts('membersList')}</SheetTitle>
            </SheetHeader>
            <ListContent
              memberships={memberships}
              online={online}
