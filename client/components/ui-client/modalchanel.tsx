...
export function ModalChanelContent() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await channelRequest(body as any);
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            <Label htmlFor="name" className="text-xs font-bold uppercase text-[#b5bac1]">Nom du Chanel</Label>
            <Input 
              id="name" 
              name="name_chanel" 
          ...
          <div className="space-y-2">
            <Label htmlFor="type_chanel" className="text-xs font-bold uppercase text-[#b5bac1]">Type de salon</Label>
            <select
              id="type_chanel" 
              name="type_chanel" 
        ...
        <DialogFooter className="bg-[#2b2d31] p-4 -m-6 mt-4 flex items-center justify-end gap-3">
          <DialogClose asChild>
            <Button type="button" variant="ghost" className="text-white hover:bg-transparent hover:underline">
              Annuler
            </Button>
          </DialogClose>
          <Button 
