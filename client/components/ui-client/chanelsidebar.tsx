...
export function ChannelSidebar({ serverId, channelId }: ChannelSidebarProps) {
  ...
  const userMembership = currentServer?.memberships?.find(
  ...
  return (
    <aside
      className={`${channelId ? "hidden" : "flex"} md:flex shrink-0 w-60 border-r border-black/20`}
    >
          ...
          {channels.map((channel) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`truncate font-medium ${isActive ? "text-white" : ""}`}
                >
                  {channel.name}
                </span>
              </Link>
