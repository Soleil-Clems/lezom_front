            ...
            >
              ...
              {isVoice && (
                <div className="relative group">
                  ...
                  <div
                    className={`flex items-center gap-3 p-3 rounded-xl min-w-[220px] max-w-[320px] ${isMyMessage ? 'bg-indigo-600' : 'bg-[#383a40]'}`}
                  >
                    <button
                    ...
                    <span
                      className={`text-xs flex-shrink-0 ${isMyMessage ? 'text-indigo-200' : 'text-zinc-400'}`}
                    >
                      🎤
                    </span>
                  </div>
              ...
              {isFile && (
                <div className="relative group">
                  ...
                  >
                    <div
                      className={`flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center ${isMyMessage ? 'bg-indigo-500 text-white' : 'bg-zinc-600 text-zinc-300'}`}
                    >
                      <FileText className="size-5" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p
                        className={`text-sm font-medium truncate ${isMyMessage ? 'text-white' : 'text-zinc-200'}`}
                      >
                        {getFileName(message.content)}
                      </p>
                      {getFileExtension(message.content) && (
                        <p
                          className={`text-xs ${isMyMessage ? 'text-indigo-200' : 'text-zinc-400'}`}
                        >
                    ...
                    <Download
                      className={`size-4 flex-shrink-0 ${isMyMessage ? 'text-indigo-200' : 'text-zinc-400'}`}
                    />
                  </a>
                </div>
              )}
