          ...
          {canDelete && (
            ...
            >
              <MessageActions
                ...
                onReact={(emoji) => {
                  onAddReaction?.(Number(message.id), emoji);
    ...
    <div className="w-full p-4 space-y-4 min-h-full">
      {messages.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-full min-h-[400px] text-center">
          ...
          <h3 className="text-xl font-semibold text-zinc-300 mb-2">{t('noMessages')}</h3>
          <p className="text-sm text-zinc-500 max-w-md">{t('beFirst')}</p>
        </div>
      ) : (
        <>
                ...
                <MessageWrapper key={message.id} message={message} isMyMessage={isMyMessage}>
                  <div
                    className={`flex items-center gap-3 p-3 rounded-xl min-w-[220px] max-w-[320px] ${
                      isMyMessage ? 'bg-indigo-600' : 'bg-[#383a40]'
                    }`}
                  >
                    <button
                  ...
                  >
                    <div
                      className={`flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center ${
                        isMyMessage ? 'bg-indigo-500 text-white' : 'bg-zinc-600 text-zinc-300'
                      }`}
                    >
